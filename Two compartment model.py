...
ksl = 0.825   # diffusion rate constant to lipid layer (d^-1)
Cw = 1.0     # concentration in water (mmol/L)

bw = 0.26     # killing rate constant (L mmol^-1 d^-1)
hb = 4.5*10**-3   # background hazard rate (d^-1)
...
Cl0 = 0.0    # initial concentration in lipid layer
S0 = 1.0      # initial survival probability

# Time span (days)
time = np.linspace(0, 2, 1000)

time2 = np.linspace(0, 20, 1000)
...
# Function for hazard rate
def hazard(bw,Cs,zw,hb):
    hz = bw * max(0,Cs - zw) + hb
    return hz

# Differential equations
def model(y, time, kws, ksw,ksl, bw, hb,zw,Cw):
    Cs, Cl, S = y
    
    # Structural and lipid concentration changes
    dCs_dt = kws * Cw - ksw * Cs - ksl * (Cs - Cl)
    dCl_dt = ksl * (Cs - Cl)
    
    hz = hazard(bw, Cs, zw, hb)
    
    # Survival change
    dS_dt = -hz * S  # negative because hazard decreases survival

    return [dCs_dt, dCl_dt, dS_dt]


args = (kws, ksw,ksl, bw, hb,zw,Cw)
y0 = [Cs0, Cl0, S0]
...
axs[0].plot(time, Cl, label='Lipid Layer Concentration ($C_l$)')
...
axs[0].plot(time2, Cl2, label='Lipid Layer Concentration ($C_l$)')
axs[0].set_ylabel('Concentration (mmol L$^{-1}$)')
axs[0].set_xlabel('Time (days)')
axs[0].legend()
axs[0].grid()
...
for i, kws in enumerate(kws_values):
    ...
    survival_table[label] = [S[np.argmin(np.abs(time - t))] for t in timepoints_days]
...
for true_ksw, est_ksw, pcov in zip(true_ksw_values, estimated_ksw_values, cov_matrices):
    ...
    p_val = 2 * (1 - t.cdf(np.abs(t_stat), df=dof))
    significant = "Yes" if p_val < 0.05 else "No"
    print(f"{true_ksw:<10.3f} {est_ksw:<10.3f} {se:<10.4f} {t_stat:<10.3f} {p_val:<10.7f} {significant}")

# ==========================================================================
#                       GET SURVIVAL DATA KSL
# ==========================================================================

kws = 1440    # uptake rate constant from water (d^-1)
ksw = 0.553    # elimination rate constant to water (d^-1)

ksl_values = [0.825, 1.65, 3.3, 6.6]
colors = plt.cm.viridis(np.linspace(0, 1, len(ksl_values)))

fig, axs = plt.subplots(4, 1, figsize=(10, 12))
survival_table = {}

for i, ksl in enumerate(ksl_values):
    args = (kws, ksw, ksl, bw, hb, zw, Cw)
    y0 = [Cs0, Cl0, S0]
    sol = odeint(model, y0, time, args=args)
...
true_ksl_values = ksl_values
observed_survival_data = [survival_table[f"ksl = {ksl}"] for ksl in ksl_values]
estimated_ksl_values = []
cov_matrices = []
...
# ==========================================================================
#                       STATISTICAL TESTING KSL
# ==========================================================================

n = len(timepoints_days)
dof = n - 1

print(f"{'True ksl':<10} {'Est. ksl':<10} {'SE':<10} {'t-stat':<10} {'p-value':<10} {'Significant?'}")
print("-" * 70)

for true_ksl, est_ksl, pcov in zip(true_ksl_values, estimated_ksl_values, cov_matrices):
    se = np.sqrt(np.diag(pcov))[0]
    t_stat = (est_ksl - true_ksl) / se
    p_val = 2 * (1 - t.cdf(np.abs(t_stat), df=dof))
    significant = "Yes" if p_val < 0.05 else "No"
    print(f"{true_ksl:<10.3f} {est_ksl:<10.3f} {se:<10.4f} {t_stat:<10.3f} {p_val:<10.7f} {significant}")
